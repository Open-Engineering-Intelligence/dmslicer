...
原始两个 solids 保留；融合使用其副本的 OCC `fuse(...).removeSplitter()`，得到新的 `U`，不以 compound 或删除整张面代替。验收记录一个连通 solid、每个实际 shell 闭合、有效性、体积守恒、输入材料包含、无额外材料、共同面不在 U（及其 STEP 重导入）边界上。A08 额外从 B-rep 确认两个封闭有效 solid、球形内壁共同面和无实质体积穿透。它的实际输入是完整小实心球与大球壳的上半部；融合后外露的小球下半球面属于外部材料边界，不是残留的封闭空腔内壁。
