...
真实支持范围仅为：两个明确角色的 body、planar/parallel/opposing/coplanar FaceSet、uniform normal gap 与 Side_2 rigid translation。不支持成员不同位移、tilt、rotation、penetration separation、曲面径向校正、sphere、cone、NURBS、mesh、field、gradient、多体 collision、自动 lateral alignment、slicing 或 G-code；遇到这些应返回结构化 `UNSUPPORTED`，不能静默降级。
