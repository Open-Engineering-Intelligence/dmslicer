...
function draw(){{ctx.clearRect(0,0,canvas.width,canvas.height);let triangles=[];scene.entities.filter(e=>e.visible).forEach(e=>e.mesh.triangles.forEach(t=>{{let a=view(e.mesh.positions[t[0]]),b=view(e.mesh.positions[t[1]]),c=view(e.mesh.positions[t[2]]);triangles.push({{e:e,a:a,b:b,c:c,depth:(a[2]+b[2]+c[2])/3}})}}));triangles.sort((left,right)=>left.depth-right.depth);triangles.forEach(item=>{{ctx.fillStyle=item.e.color;ctx.globalAlpha=item.e.entity_kind==='PATCH'?.82:1;ctx.beginPath();ctx.moveTo(item.a[0],item.a[1]);ctx.lineTo(item.b[0],item.b[1]);ctx.lineTo(item.c[0],item.c[1]);ctx.closePath();ctx.fill()}});ctx.globalAlpha=1}}
