...
<style>body{{font:14px system-ui;display:grid;grid-template-columns:1fr 320px;margin:0}}canvas{{width:100%;height:100vh;background:#111}}aside{{padding:12px;overflow:auto}}button{{margin:3px;text-align:left}}label{{display:block;margin:4px 0}}details{{border-top:1px solid #ddd;padding:7px 0}}summary{{font-weight:700;cursor:pointer}}#trace{{white-space:pre-wrap;font-size:11px}}#filter{{width:96%;padding:6px}}</style>
<canvas id="scene" width="900" height="650"></canvas><aside><h1>CASE01 display-only B-rep mesh</h1><p>Visibility changes only the display mesh; it does not union Geometry or permit path crossing.</p><input id="filter" placeholder="Filter objects"><div id="controls"></div><h2>Selection traceability</h2><pre id="trace">No entity selected.</pre></aside>
<script>const scene={payload}, canvas=document.querySelector('#scene'), ctx=canvas.getContext('2d'), controls=document.querySelector('#controls'), trace=document.querySelector('#trace'), filter=document.querySelector('#filter');let selected=null,drag=null,azimuth=-.72,elevation=.44,zoom=15;
scene.entities.forEach((e,i)=>{{e.visible=true;e.color=e.entity_kind==='PATCH'?'#ffd166':['#4cc9f0','#80ed99','#f72585'][i%3]}});function group(title,items){{let d=document.createElement('details');d.open=true;let s=document.createElement('summary');s.textContent=title+' ('+items.length+')';d.append(s);let all=document.createElement('button'),none=document.createElement('button');all.textContent='All';none.textContent='None';all.onclick=()=>{{items.forEach(e=>e.visible=true);renderControls();draw()}};none.onclick=()=>{{items.forEach(e=>e.visible=false);renderControls();draw()}};d.append(all,none);items.forEach(e=>{{let l=document.createElement('label'),c=document.createElement('input');c.type='checkbox';c.checked=e.visible;c.onchange=()=>{{e.visible=c.checked;draw()}};l.append(c,' ',e.display_name);l.onclick=event=>{{if(event.target!==c)select(e)}};d.append(l)}});controls.append(d)}}function renderControls(){{controls.replaceChildren();let query=filter.value.toLowerCase(),matches=e=>e.display_name.toLowerCase().includes(query);group('Regions',scene.entities.filter(e=>e.entity_kind==='REGION'&&matches(e)));group('Interfaces',[]);group('Patches',scene.entities.filter(e=>e.entity_kind==='PATCH'&&matches(e)))}}filter.oninput=renderControls;renderControls();
...
function normal(a,b,c){{let u=[b[0]-a[0],b[1]-a[1],b[2]-a[2]],v=[c[0]-a[0],c[1]-a[1],c[2]-a[2]],n=[u[1]*v[2]-u[2]*v[1],u[2]*v[0]-u[0]*v[2],u[0]*v[1]-u[1]*v[0]],m=Math.hypot(...n)||1;return n.map(x=>x/m)}}function shade(color,n){{let l=Math.abs(n[0]*-.4+n[1]*-.5+n[2]*.75),s=.30+.70*l,h=color.slice(1),r=parseInt(h.slice(0,2),16),g=parseInt(h.slice(2,4),16),b=parseInt(h.slice(4,6),16);return `rgb(${{Math.round(r*s)}},${{Math.round(g*s)}},${{Math.round(b*s)}})`}}function facetEdges(e){{let map=new Map;for(let t of e.mesh.triangles){{let n=normal(e.mesh.positions[t[0]],e.mesh.positions[t[1]],e.mesh.positions[t[2]]);for(let edge of [[t[0],t[1]],[t[1],t[2]],[t[2],t[0]]]){{let a=edge[0],b=edge[1],key=a<b?`${{a}}:${{b}}`:`${{b}}:${{a}}`,item=map.get(key)||{{a:a,b:b,normals:[]}};item.normals.push(n);map.set(key,item)}}}}return [...map.values()].filter(item=>item.normals.length===1||Math.abs(item.normals[0][0]*item.normals[1][0]+item.normals[0][1]*item.normals[1][1]+item.normals[0][2]*item.normals[1][2])<.999)}}
