...
const WorkspaceAnnotations = (() => {
 ...
 const roles=['Unassigned','Isolated region','Source','Gradient'];
 ...
 function library(){return {schema:'dmslicer.material-library.v1',version:0,materials:[],history:[]}}
 function create(c,materials=library()){
  ...
  return {schema:'dmslicer.workspace-annotation.v1',source:source(c),created_at:now(),saved_at:null,
   material_library:copy(materials),material_library_version:materials.version,activation:copy(activation),decisions:[],
   objects:c.scene.entities.filter(e=>e.identity_status==='STABLE_REFERENCE').map(e=>({entity_ref:e.entity_ref,
    reference_provenance:copy(e.reference_provenance),source_geometry:copy(e.source||null),geometry_role:geometryRole(e),semantic_role:'Unassigned',material_id:null,display_override:null}))};
 }
 function validateMaterial(m){
  ...
  function finite(v){if(typeof v==='number')require(Number.isFinite(v),'Properties must be finite');else if(object(v)||Array.isArray(v))Object.values(v).forEach(finite)}finite(m.properties);
 }
 ...
 function validate(c,w){
  require(object(w)&&w.schema==='dmslicer.workspace-annotation.v1','Invalid workspace annotation schema');
  ...
  require(Array.isArray(w.decisions)&&w.decisions.every(d=>object(d)&&d.actor==='human'&&typeof d.at==='string'&&Number.isFinite(Date.parse(d.at))),'Invalid decision history');
  ...
  for(const o of w.objects){const e=refs.get(o.entity_ref);require(e,'Unknown/duplicate stable reference');refs.delete(o.entity_ref);
   ...
   require(o.geometry_role===e.geometry_role,'Geometry role is read only');require(roles.includes(o.semantic_role),'Invalid semantic role');
   if(e.geometry_role!=='Input')require(o.semantic_role==='Unassigned'&&o.material_id===null&&o.display_override===null,'Only input block/region annotations are supported');
 ...
 function decide(w,ref,field,value){
  ...
  require(['semantic_role','material_id','display_override'].includes(field),'Geometry role is read only');
  if(field==='semantic_role')require(roles.includes(value),'Invalid semantic role');
  ...
  w.decisions.push({actor:'human',at:now(),entity_ref:ref,field,before:o[field],after:value,material_library_version:w.material_library.version});o[field]=value;
 }
 function putMaterial(w,data){
  validateMaterial(data);const l=w.material_library,old=l.materials.find(m=>m.id===data.id),at=now();
  const value={...copy(data),updated_at:at};
 ...
 function restore(c,text){require(typeof text==='string'&&text.length<=4*1024*1024,'Annotation limit: 4 MiB');return validate(c,JSON.parse(text))}
 ...
 function key(c){return 'dmslicer.workspace.v1:'+canonical(source(c))}
 ...
 return {create,geometryRole,roles,library,validateLibrary,putMaterial,decide,color,validate,restore,replaceLibrary,key,save};
})();
