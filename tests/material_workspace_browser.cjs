...
(async()=>{
 ...
 try{
  ...
  await check('material add returns to source row without assignment; role material color independent',async()=>{
   ...
   const row=page.locator('.annotation-row').first();assert(await page.locator('.annotation-row').filter({hasText:'Patch · Geometry role'}).locator('.semantic-role').first().isDisabled());assert.equal(await row.locator('.semantic-role').inputValue(),'Unassigned');
  ...
  await check('explicit save and portable export preserve fields and decisions',async()=>{
   ...
   await page.reload();await page.waitForFunction(()=>document.documentElement.dataset.ready==='true');await sample('case01');await page.getByRole('tab',{name:'材料与语义配置',exact:true}).click();assert.equal(await page.locator('.semantic-role').first().inputValue(),'Gradient');
  });
  await check('save failure retains draft and recovery; imported data source checked',async()=>{
   ...
   await page.locator('#save-workspace').click();assert((await page.locator('#annotation-status').textContent()).includes('已保存'));
   await page.locator('#import-workspace').setInputFiles(exported);await page.locator('#annotation-status').filter({hasText:'已导入'}).waitFor();assert.equal(await page.locator('.semantic-role').first().inputValue(),'Gradient');await page.locator('#save-workspace').click();
   await sample('c02');await page.locator('#import-workspace').setInputFiles(exported);await page.locator('#annotation-status').filter({hasText:'导入失败'}).waitFor();assert((await page.locator('#annotation-status').textContent()).includes('失败'));assert.equal(await page.locator('.semantic-role').first().inputValue(),'Unassigned');
  });
  ...
  await check('library edits retain key and assignments; duplicate add rejected; keyboard tabs reachable',async()=>{
   ...
   await page.locator('#material-list button').first().click();assert(await page.locator('#material-id').getAttribute('readonly')!==null);
   await page.locator('#material-name').fill('聚合物 A（编辑）');await page.locator('#material-color').fill('#336699');await page.locator('#material-description').fill('修订说明');await page.locator('#material-properties').fill('{"density":{"value":1.3,"unit":"g/cm3"}}');
   await page.getByRole('button',{name:'保存材料',exact:true}).click();
   await page.getByRole('tab',{name:'材料与语义配置',exact:true}).click();
   assert.equal(await page.locator('.material-assignment').first().inputValue(),'test-polymer');assert.equal(await page.locator('.semantic-role').first().inputValue(),'Gradient');assert.equal(await page.locator('.display-color').first().inputValue(),'#336699');
   ...
   await page.locator('#new-material').click();await page.locator('#material-id').fill('bad-json');await page.locator('#material-name').fill('坏属性');await page.locator('#material-properties').fill('[]');await page.getByRole('button',{name:'保存材料',exact:true}).click();assert((await page.locator('#library-status').textContent()).includes('失败'));assert.equal(await page.locator('#material-list button').count(),1);
  ...
  await check('standalone library export/import validates before replacing domain snapshot',async()=>{
   ...
   const lib=JSON.parse(fs.readFileSync(filename));assert.equal(lib.version,2);assert.equal(lib.materials[0].name,'聚合物 A（编辑）');assert.equal(lib.materials[0].properties.density.value,1.3);
   ...
   await page.locator('#import-library').setInputFiles(invalid);await page.locator('#library-status').filter({hasText:'导入失败'}).waitFor();assert.equal(await page.locator('#material-list button').count(),1);
   await page.locator('#save-library').click();await page.getByRole('tab',{name:'材料与语义配置',exact:true}).click();await page.locator('#save-workspace').click();
  ...
  await check('missing annotation provenance degrades to read-only without breaking geometry viewer',async()=>{
   ...
   assert(await page.locator('.semantic-role').first().isDisabled());assert(await page.locator('#save-workspace').isDisabled());assert((await page.locator('#annotation-status').textContent()).includes('不可用'));
